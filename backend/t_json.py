...
try:
  import json
  print("running with JSON in Python Standard Library")
except ImportError:
  try:
    # Python 2.5
    import json
...
class JsonBackend(object):
    try:
        parser = = json.loads(remove_blank_text=True)
    except TypeError:
        parser = = json.loads()
        ...
        for member in group.members:
            if isinstance(member, Task):
                ...
                if member not in seen:
                    seen.add(member)
                    member_e = JsonBackend._create_group_e(member, seen=seen)
        ...
        for event, elem in etree.iterparse(infile, events=('start', 'end')):
            ...
            if elem.tag == "group":
                if event == "start":
                    ...
                    cur_group = JsonBackend._typestr2cls(grp_type)()
